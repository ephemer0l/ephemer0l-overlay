...
for docname in "${ARDUINO_DOCS[@]}"; do
	ARDUINO_DOCS_URI+=" https://downloads.arduino.cc/${docname}.tar.gz -> ${PN}-${docname}.zip"
done

SRC_URI="https://github.com/arduino/Arduino/archive/${PV}.tar.gz -> ${P}.tar.gz
	https://downloads.arduino.cc/cores/avr-1.8.2.tar.bz2 -> ${PN}-avr-1.8.2.tar.bz2
	https://github.com/arduino-libraries/WiFi101-FirmwareUpdater-Plugin/releases/download/v0.10.10/WiFi101-Updater-ArduinoIDE-Plugin-0.10.10.zip -> ${PN}-WiFi101-Updater-ArduinoIDE-Plugin-0.10.10.zip
	doc? (
		${ARDUINO_DOCS_URI}
	)"
...
src_unpack() {
	...
	cp "${DISTDIR}/${PN}-avr-1.8.2.tar.bz2" "${S}/build/avr-1.8.2.tar.bz2" || die
	cp "${DISTDIR}/${PN}-WiFi101-Updater-ArduinoIDE-Plugin-0.10.10.zip" "${S}/build/shared/WiFi101-Updater-ArduinoIDE-Plugin-0.10.10.zip"|| die

	if use doc; then
		local docname
		for docname in "${ARDUINO_DOCS[@]}"; do
			cp "${DISTDIR}/${PN}-${docname}.tar.gz" "${S}/build/shared/${docname}.tar.gz" || die
		done
	fi
}
