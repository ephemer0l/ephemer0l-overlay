...
if [ "${PV}" == "9999" ]; then
	inherit git-r3
	EGIT_REPO_URI="https://github.com/signalwire/freeswitch.git"
	KEYWORDS=""
elif [ "${PV}" == "0_p20191230" ]; then
	_MY_COMMIT="cdaebdce9df2a9d69912f1e027be3547c304e5f2"
	SRC_URI="https://github.com/signalwire/freeswitch/archive/"${_MY_COMMIT}".tar.gz -> "${P}".tar.gz"
	KEYWORDS="~amd64 ~x86"
	#S=${WORKDIR}/${_GIT_USER}-${PN}-${_GIT_COMMIT}
else
	SRC_URI="https://github.com/signalwire/freeswitch/archive/v"${PV}".tar.gz -> "${P}".tar.gz"
	KEYWORDS="~amd64 ~x86"
fi
...
RDEPEND="
	...
	freeswitch_modules_cdr_pg_csv? ( >=dev-db/postgresql-1.0.0:* )
	...
	freeswitch_modules_h323? ( || ( net-libs/h323plus net-libs/ptlib ) )
	...
	freeswitch_modules_skypopen? ( x11-base/xorg-server x11-apps/xhost net-im/skypeforlinux media-fonts/font-misc-misc media-fonts/font-cursor-misc )
